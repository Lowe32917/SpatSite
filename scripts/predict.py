...
def parse_args():
    ...
    parser.add_argument("--test_name", dest='test_name', default='Spat_predict', help='Name of test datasets.')
    # model parameter
...
class Config():
    def __init__(self, args):
        ...
        self.output_folder = os.path.abspath('..') + '/results/'
        if not os.path.exists(self.output_folder): os.makedirs(self.output_folder)

        self.log_output_folder = os.path.abspath('..') + '/checkpoints/'
...
def main(opt,device):
    ...
    predict_site = predict_probs > th

    pro_list = list(set(pro_name_list))

    with open(opt.output_folder + '/' + opt.dataset_name + '_predict.txt', 'w') as file:
        for pro in pro_list:
            file.write(f">{pro}\n")
...
if __name__ == '__main__':
    ...
    sys.stdout = Logger(opt.log_output_folder + '/' + opt.dataset_name + '_predict.log')
